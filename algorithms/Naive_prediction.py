...
class Bayes:
    ...
    #     print(pos[0:10,0:10],"pos")
        pos_prob=np.sum(np.log(pos),1)+np.sum(np.log(prior_p))
        neg_prob=np.sum(np.log(neg),1)+np.sum(np.log(prior_n))
    ...
    def train(self,X,label,X_val=None,label_val=None,X_test=None,label_test=None,param=None,lambda_para=1):
...
#         sys.stdout=Logger(directory+"log.txt")
    ...
    #     results={"W":W_best,"b": b_best , "metrics":best_val  }
        results={"param":[Naive_param_np ,prior_p,prior_n]}
        final=self.predict(X_val,label_val,results,param)
