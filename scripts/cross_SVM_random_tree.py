...
#                 train_x,train_label,val_x,val_label=next(gene)
    #             print(train_label.shape,val_label.shape,)
        ...
        #             print(train_label.shape,val_label.shape,)
        #         argg.eta_1=lr[k]
                argg.k_trees=k_trees[o]

                results_val_forest=Random_forest(train_x,train_label,val_x,val_label,parse=argg) 
                train_x=Random_forest_prediction(train_x,train_label,results_val_forest,parse=argg)["feature"]
                val_x=Random_forest_prediction(val_x,val_label,results_val_forest,parse=argg)["feature"]
    ...
    #             print(argg.metrics+" as "+str(results_val["metrics"])+" for validating when lr1 is "+str(lr[k])+" and C_tradeoff is "+str(C_trdeoff[l])+" at fold "+str(m))
            ...
            mean=per_ech.mean()
            std=per_ech.std()
            print("validating average "+argg.metrics+" as "+str(mean)+" when lr1 is "+str(lr[k])+" and C_tradeoff is "+str(C_trdeoff[l])," k_trees is "+str(k_trees[o]))
            if  mean>=best_f1:
                best_lr1=lr[k]
                best_C_trdeoff=C_trdeoff[l]
                best_f1=mean
                best_k_tree=k_trees[o]
                best_tree=results_val
                best_forest=results_val_forest
                print("Found a better tree "+"with "+argg.metrics+" as "+str(best_f1)+" when lr1 is "+str(best_lr1)+" and C_tradeoff is "+str(best_C_trdeoff)+" k_trees is "+str(k_trees[o]))
...
print("best_lr1 is",best_lr1," best_C_trdeoff" ,best_C_trdeoff," best k_tree ",best_k_tree, "which we can get " ,argg.metrics+" as "+str(best_f1)+"for validating")
argg.eta_1=best_lr1
argg.C_trdeoff=best_C_trdeoff
argg.k_trees=best_k_tree
...
train_x=Random_forest_prediction(data.X_train,train_label,best_forest,parse=argg)["feature"]
# test_x=Random_forest_prediction(data.X_test,data.label_test,best_forest,parse=argg)["feature"]
results_eval=calculate_metrics_prediction(train_x,data.label_train,best_tree,argg)
print("*******************************final training performance is ",results_eval["metrics"],"******************************")

test_x=Random_forest_prediction(data.X_test,data.label_test,best_forest,parse=argg)["feature"]
results_eval=calculate_metrics_prediction(test_x,data.label_test,best_tree,argg)
print("*******************************final test performance is ",results_eval["metrics"],"******************************")




# results_val=SVM(test_x,data.label_test,train_x,data.label_train,param=argg)
