...
argg=arg.parse_args(["--train_data", "/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_semeion/hand_data_train", "--val_data","/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_semeion/hand_data_train","--test_data", "/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_semeion/hand_data_test","--json_file","../../setting_Naive_bayes.json","-e1",".1","-C"," .1","--maxium_epoch",' 40',"--norm","--metrics","acc","--norm","--binary"])

# argg=arg.parse_args(["--train_data", "/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_madelon/madelon_data_train", "--val_data","/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_semeion/hand_data_train","--test_data", "/home/taoyang/research/research_everyday/homework/ml6350/hw5/data/data_madelon/madelon_data_test","--json_file","../../setting_Naive_bayes.json","-e1",".1","-C"," .1","--maxium_epoch",' 80',"--metrics","acc","--norm","--binary"])
...
print(setting)
directory=argg.log_file+"current_Naive_bayes"+str(datetime.now())+"/"
if not os.path.exists(directory):
    os.makedirs(directory)
sys.stdout=Logger(directory+"log.txt")
print(argg)
...
for l in  range(len(smooth_term)):
...
#         argg.eta_1=lr[k]
        argg.smooth_term=smooth_term[l]

        results_val=Naive_bayes(train_x,train_label,val_x,val_label,param=argg)
...
#             print(argg.metrics+" as "+str(results_val["metrics"])+" for validating when lr1 is "+str(lr[k])+" and C_tradeoff is "+str(C_trdeoff[l])+" at fold "+str(m))
    ...
    mean=per_ech.mean()
    std=per_ech.std()
    print("validating average "+argg.metrics+" as "+str(mean)+" when smooth term is "+str(smooth_term[l]))
    if  mean>=best_f1:
        best_smooth_term=smooth_term[l]
#         best_C_trdeoff=C_trdeoff[l]
        best_f1=mean
        best_tree=results_val
        print("Found a better tree "+"with "+argg.metrics+" as "+str(best_f1)+" when smooth term is "+str(smooth_term[l]))
...
print("best_smooth_term" ,best_smooth_term, "which we can get " ,argg.metrics+" as "+str(best_f1)+"for validating")

argg.smooth_term=best_smooth_term
...
results_eval=Naive_prediction(data.X_train,data.label_train,best_tree,argg)
print("*******************************final training performance is ",results_eval["metrics"],"******************************")

results_eval=Naive_prediction(data.X_test,data.label_test,best_tree,argg)
...
os.rename(directory,argg.log_file+"naive_bayes cross_validate_lr1 "+"smooth_term"+str(setting['smooth_term'])+"precision"+str(best_f1)+" metircs"+argg.metrics+"    "+str(datetime.now()))